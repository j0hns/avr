...
int USART_ReceiveBytesAvailable(void)
{
	return ((unsigned int)(USART_RX_BUFFER_SIZE + USART_RxHead - USART_RxTail)) % USART_RX_BUFFER_SIZE;
}
...
int USART_Peek(void)
{
	if (USART_ReceiveIsEmpty()) {
		return -1;
		} else {
		return USART_RxBuf[USART_RxTail];
	}
}